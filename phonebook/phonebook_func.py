...
#               using Tkinter Parent and Chilr relationships
#
#
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): #enforces the user to put both names
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cursor = conn.cursor()
            ...
            chkName = count
            if chkName == 0: # if this is - then there is not existence of the fullname and we can add new data
        ...
        messagebox.showerror("Missing Text Error", "Please ensure that thre is data in all four fields.")
...
def onDelete(self):
    ...
    with conn:
        cur = conn.cursor()
        # check count to ensure that this isnot the last record in
...
def onUpdate(self):
    ...
    # for name changes the user will need to delete the entirerecord and start over
    var_phone = self.txt_phone.get().strip()
    var_email = self.txt_email.get().strip()
    if (len(var_phone) > 0) and (len(var_email) > 0):  # ensure there is data present
        ...
        with conn:
            cur = conn.cursor()
            # count records to see if the users changes are already in
        ...
        messagebox.showerror("Missing info", "Please select a name from the lsit. \nThen edit the phone or email info.")
    onClear(self)
